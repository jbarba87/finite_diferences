...
Heat::Heat(float _dt, float _dx, float _t_end, float _x_end, float _coef, float *_b, Vector (*_t0)(Vector) ){
...
void Heat::Heat_Solve(){

  float r = this->coef * this->dt / (this->dx * this->dx);
  ...
  for (i = 0; i < (this->NTimeVectors) - 1; i++){
    
    future->setData(b[0], 0);
    future->setData(b[1], NPosElements);

    for (j = 1; j < (this->NPosElements) - 1; j++){

      aux = r*present->getData(j-1) + (1-2*r)*present->getData(j) + r*present->getData(j+1);
...
Vector *Heat::getTimeVector(int pos){
...
void Heat::Heat_Plot(int max_height){
...
void Heat::Heat_nPlot(int max_height){
